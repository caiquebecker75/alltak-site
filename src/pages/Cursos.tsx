          ...
          {CURSOS.map((c, i) => (
            <Reveal key={c.nome} delay={i * 60}>
              <div className="flex h-full flex-col justify-between border border-black/10 bg-white p-7">
                <div>
                  <span className="font-display text-xs font-bold uppercase tracking-widest text-alltak-red">{c.nivel}</span>
                  <h3 className="mt-2 text-2xl">{c.nome}</h3>
                  <p className="mt-2 text-sm text-alltak-black/60">{c.desc}</p>
                </div>
                <button className="btn-trapezoid btn-red mt-6 self-start !py-2 !text-xs">Tenho interesse</button>
              </div>
            </Reveal>
          ))}
        </div>
